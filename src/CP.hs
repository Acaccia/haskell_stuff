...
liftISO2 (ab, ba) = (\aaa x y -> ab $ aaa (ba x) (ba y), \bbb x y -> ba $ bbb (ab x) (ab y))
...
instance Nat [()] where
  ...
  nat z _ [] = z
  nat _ s (_:n) = s n
  iter z f = foldl (const . f) z
  plus = (++)
  minus = (\\)
  mult = (*>)
  n `pow` m = foldl mult 1 (n <$ m)
...
instance Nat Scott where
  zero = Scott const
  successor n = Scott $ const ($ n)
  nat z f (Scott n) = n z f
  iter z f (Scott n) = n z (f . iter z f)
...
instance Nat Church where
  ...
  nat z f (Church n) = undefined --     maybe z f $ x (Just . maybe zero successor) Nothing
  iter z f (Church n) = n f z
  ...
  Church n `plus` Church m = Church ((.) <$> n <*> m)
  n `minus` Church m = m (nat 0 id) n
  Church n `mult` Church m = Church (n . m)
