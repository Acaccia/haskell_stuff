...
class Functor f => Pretty f where
  prettify :: Pretty g => f (Expr g) -> String

pretty :: Pretty f => Expr f -> String
pretty (In e) = prettify e

instance Pretty Lit where
  prettify (Lit a) = show a

instance Pretty Add where
  prettify (Add a b) = "(" ++ pretty a ++ "+" ++ pretty b ++ ")"

instance Pretty Mult where
  prettify (Mult a b) = "(" ++ pretty a ++ "*" ++ pretty b ++ ")"

instance (Pretty f, Pretty g) => Pretty (f :+: g) where
  prettify (Inl a) = prettify a
  prettify (Inr b) = prettify b
